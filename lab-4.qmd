...
checkboxGroupInput('region', 'Select Regions:',
                         choices = unique(cleaned_dataset$New_region),
                         selected = unique(cleaned_dataset$New_region))

server <- function(input, output, session) {
  
  filtered_countries <- reactive({
    data <- cleaned_dataset[cleaned_dataset$New_region %in% input$region, ]
    return(data)
  })
}
```
