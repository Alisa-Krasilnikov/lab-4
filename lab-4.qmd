...
```{r}
valueBoxOutput("scienceValueBox", width = 8)
```
...
output$scienceValueBox <- renderValueBox({
  valueBox(
    value = overall_science_mean()$mean_sci_trust,
    color = "maroon",
    subtitle = "Overall Average Science Trust Index"
  )
})
```

```{r}
valueBoxOutput("medValueBox", width = 8)
```

```{r}
#| context: server

output$medValueBox <- renderValueBox({
  valueBox(
    value = paste0(overall_mean()$mean_trust, "%"),
    color = "purple",
    subtitle = "Overall Average Medical Trust"
  )
})
```
